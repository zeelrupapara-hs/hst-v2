...
func (s *HttpServer) ServeWS(c *websocket.Conn) {
	...
	agent, _ := c.Locals(nethttp.LocalsUserAgent).(string)
	...
	}, ip, utils.OperatingSystem(agent))
...
func (s *HttpServer) JournalEntry(c *fiber.Ctx, typ model.JournalType, code logger.Code, message string, detail any) {
	...
	channel, os := model.ChannelSystem, ""
	if snap != nil {
		channel = model.UsersConnectionTypes(snap.ConnectionType).Channel()
		os = utils.OperatingSystem(utils.GetUserAgent(c))
	}

	s.WriteJournalFrom(c.UserContext(), login, utils.GetRealIP(c), channel, os, typ, code, message, detail)
}
