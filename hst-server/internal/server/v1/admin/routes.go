...
func (s *Server) RegisterAdminV1(api, root fiber.Router) {
	...
	symbols.Get("/folders", s.Middleware.Authorization(model.MgrRightCfgSymbols), s.ListSymbolFolders)
	symbols.Post("/folders", s.Middleware.Authorization(model.MgrRightCfgSymbols), s.CreateSymbolFolder)
