  ...
  function handleMove(clientX, trackEl) {
    ...
    onChange(
      windows.map((w, i) => {
        ...
        return dragRef.current.edge === "open"
          ? { ...w, open: Math.min(m, w.close - 1) }
          : { ...w, close: Math.max(m, w.open + 1) };
      }),
    );
  }
            ...
            <div key={i}>
              ...
              {!readOnly &&
                ["open", "close"].map((edge) => (
                  ...
                  >
                    <span className="sym-timeline-marker-label">{minutesToTime(w[edge])}</span>
...
export function SessionEditorDialog({ symbol, dayIndexes, sessions, onSave, onClose }) {
  ...
  return (
    <div className="sym-session-dialog-overlay" onClick={onClose}>
      <div className="sym-session-dialog" role="dialog" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sym-session-dialog-body">
          <div className="sym-sessions-intro sym-sessions-intro-compact">
            <span className="sym-tab-intro-icon sym-tab-intro-icon-sm" aria-hidden="true">
              <Icon id="time" size={40} />
            </span>
            <p>
              The setting up of sessions within the selected day. Trading sessions must be within
              the quotation ones.
            </p>
          </div>
          <TimelineRow label="Quotes:" windows={quoteWins} onChange={setQuoteWins} variant="quote" />
          <TimelineRow
            label="Trade:"
