...
func New(cfg *config.Config, log *logger.Logger, nc *nats.Nats, rds *redis.Redis, influx *influxdb.Client) *Handler {
	h := &Handler{
		Cfg:     cfg,
		Log:     log,
		Nats:    nc,
...
func (h *Handler) runConfigReload(ctx context.Context) {
	interval := h.Cfg.Quote.ReloadInterval
	if interval <= 0 {
		return
	}
